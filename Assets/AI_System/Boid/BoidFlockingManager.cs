...
public class BoidFlockingManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (m_Incombat)
        {
            desiredVelocity = InCombat(nearbyAllies, nearbyEnemies, targetEnemy, movSpeed, slowRadius, stopRange, visRange);
        }
        else
        {
        ...
        {
            desiredVelocity += SteeringBehaviours.Cohesion(formationPos, m_Rigidbody.position, visRange, movSpeed) * formationWeight;
        }
    ...
    private Vector3 OutOfCombat(Dictionary<Guid, Rigidbody> _NearbyAllies, Dictionary<Guid, Rigidbody> _NearbyEnemies, KeyValuePair<Guid, Rigidbody> _TargetEnemy, Vector3 _MovTarget, float _MovSpeed, float _SlowRadius, float _StopRange, float _VisRange)
    {
        ...
        {
            if (Vector3.Distance(_TargetEnemy.Value.position, m_Rigidbody.position) < _StopRange * 2)
            {
                m_Incombat = true;
            }
    ...
    private Vector3 InCombat(Dictionary<Guid, Rigidbody> _NearbyAllies, Dictionary<Guid, Rigidbody> _NearbyEnemies, KeyValuePair<Guid, Rigidbody> _TargetEnemy, float _MovSpeed, float _SlowRadius, float _StopRange, float _VisRange)
    {
